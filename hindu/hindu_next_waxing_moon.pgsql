-------------------------------------------------------------------------------
-- Find the next waxing (quarter) moon after a given JDE (stored in seconds)
-- 
...
--
-- Returns the JDE of the instant of the closest waxing moon forward (at
-- midnight UTC, shifted by beijing timezone)
-------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION calendars.hindu_next_waxing_moon(
...
BEGIN
	...
	-- JDE at midnight
	t_next_waxing_moon := calendars.midnight_offset(
		astronomia.moonphase_first_quarter_moon(
	...
	LOOP
		t_count := t_count + 1;
		EXIT WHEN t_next_waxing_moon >= t_jde OR t_count >= 4;
		t_next_waxing_moon := calendars.midnight_offset(
			astronomia.moonphase_first_quarter_moon(
				astronomia.jde_to_julian_year(
	...
	END LOOP;
    RETURN astronomia.jde_to_gregorian(t_next_waxing_moon)::DATE;
END;
