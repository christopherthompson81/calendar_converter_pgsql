...
-------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION calendars.get_next_new_moon(
	p_jde NUMERIC
)
RETURNS NUMERIC
AS $$
...
DECLARE
	...
	-- JDE at midnight
	t_next_new_moon NUMERIC := calendars.beijing_midnight(
...
BEGIN
	LOOP
		t_count := t_count + 1;
		EXIT WHEN t_next_new_moon >= p_jde OR t_count >= 4;
		t_next_new_moon := calendars.beijing_midnight(
			astronomia.moonphase_new_moon(
				astronomia.jde_to_julian_year(
